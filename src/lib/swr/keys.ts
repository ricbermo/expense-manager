...
export const swrKeys = {
  ...
  accountActivity: (month: string) =>
    [swrKeyPrefix.accounts, "activity", month] as const,
  budgets: (month: string) => [swrKeyPrefix.budgets, month] as const,
  categories: (type?: CategoryType) =>
    [swrKeyPrefix.categories, type ?? "all"] as const,
  dashboard: (month: string) => [swrKeyPrefix.dashboard, month] as const,
  monthlyComparison: () =>
    [swrKeyPrefix.dashboard, "monthly-comparison"] as const,
  transactions: (month?: string) =>
    [swrKeyPrefix.transactions, month ?? "all"] as const,
